...
(() => {
  ...
  // .wrangler/tmp/bundle-CsjldW/checked-fetch.js
...
`
  ...
  // .wrangler/tmp/bundle-CsjldW/middleware-insertion-facade.js
  ...
  async function generateEmailAddress() {
    ...
    return `${username}@your-domain.workers.dev`;
  }
  __name(generateEmailAddress, "generateEmailAddress");
  async function handleNewEmail(request) {
    const email = await request.json();
    const timestamp = (/* @__PURE__ */ new Date()).getTime();
    const emailId = `email:${timestamp}`;
    await TEMP_EMAIL.put(emailId, JSON.stringify(email), {
      expirationTtl: 86400
      // 24 hours
    });
  ...
  __name(handleNewEmail, "handleNewEmail");
  async function getEmails(request) {
    const { searchParams } = new URL(request.url);
    const address = searchParams.get("address");
    const emails = [];
    const emailList = await TEMP_EMAIL.list({ prefix: `email:` });
    for (const key of emailList.keys) {
      const email = await TEMP_EMAIL.get(key.name);
      if (email) {
        const emailData = JSON.parse(email);
        if (emailData.to === address) {
          emails.push({
            id: key.name.split(":")[1],
            ...emailData
          });
